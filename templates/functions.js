...
function sendLogin() {
    const chatBox = document.getElementById("Login-form-comment");
    const Login = chatBox.value;
    const passBox = document.getElementById("pass-form-comment");
    const Pass = passBox.value;
    chatBox.value = "";
    chatBox.focus();
    if (comment !== "") {
        socket.send(JSON.stringify({'messageType': 'loginInfo', 'Login': Login, 'Password': Pass}));
    }
}
